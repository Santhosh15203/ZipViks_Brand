...
export default function Tpant({setItem}){
      const [loading, setLoading] = useState(true);
      const [products, setProducts] = useState([]);
    
      const [page, setPage] = useState(1);
      const [hasMore, setHasMore] = useState(true);
      const[productType,setProductType]=useState("tpant")
      ...
      useEffect(() => {
        ...
        fetch(
          `${import.meta.env.VITE_REACT_APP_PRODUCT_URL}/tpant?page=${page}&limit=12`,{ signal }
        )
          .then((res) => res.json())
          .then((res) => {
            if (page === 1) {
              setProducts(res.tpantproducts || []);
            } else {
              setProducts((prev) => [...prev, ...(res.tpantproducts || [])]);
            }
                  ...
                  <div className=" mt-0 row">
                    {loading && page === 1 ? (
                      ...
                      <>
                        ...
                        <h5 className="text-center mt-3 fw-bold" style={{ fontFamily: "'Nunito Rounded', sans-serif"}}>Latest Track Pant Collections</h5>
